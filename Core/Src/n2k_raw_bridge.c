...
/* Debug-UART decoders removed.  RAW_BRIDGE_DEBUG_UART was always 0 in production
 * and the entire #if-block was dead.  Identity PGNs are reassembled by
 * raw_bridge_identity_frame_process() and consumed via the assembled-event FIFO. */
...
static void raw_bridge_handle_spi_packet(const uint8_t *packet, uint16_t packet_len) {
    ...
    pkt_type = packet[2];
    /* Refuse SPI_PACKET_TYPE_N2K_TX_FRAME outright.  This bridge is an
     * anonymous lurker (never claims an N2K source address); any frame
     * injected from the ESP32 would be transmitted with our NULL/static
     * source and would either be rejected by the bus or appear as a
     * non-compliant transmission.  If the ESP32 ever needs to inject
     * N2K traffic, it must do so as its own claimed device, not via
     * this bridge.  Accept only N2K_RX_FRAME (used by sensor build,
     * see SDolve sensor template) until that capability returns. */
    if (pkt_type == SPI_PACKET_TYPE_N2K_TX_FRAME) {
        s_stats.spi_parse_errors++;
        return;
    }
    if (pkt_type == SPI_PACKET_TYPE_N2K_RX_FRAME) {
        if (!SPI_Packet_ParseFramePacket(packet, packet_len, pkt_type, &frame)) {
            s_stats.spi_parse_errors++;
            return;
        }
        s_stats.spi_rx_packets++;
        /* RX frames coming in over SPI are observation-only here; the
         * direction-bit is already set by the originating sensor. */
        return;
    }
